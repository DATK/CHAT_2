...
class User:
    ...
    def reg_user(self, name):
        if self.have_user():
            ...
            self.usr = {"login": self.login, "password": self.password, "rules": {
                "READ": 1, "WRITE": 1, "CREATE": 1, "CREATE_MAX": 2, "CREATE_NOW": 0}, "parm": {"name": name}}
            with open(self.path_user, "w", encoding="UTF-8") as f:
                json.dump(self.usr, f)
            return ("Succses", 1)

    def create_room(self):
        self.usr["rules"]["CREATE_NOW"] += 1
        with open(self.path_user, "w", encoding="UTF-8") as f:
            json.dump(self.usr, f)
    ...
    def chek_reg(self):
        if self.have_user():
            ...
            if tmp_us["password"] == self.password and tmp_us["login"] == self.login:
                ...
                return ("LoginOrPasswordIsUncorrect", 0)
        else:
            return ("NoThisUser", 0)
